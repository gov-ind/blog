...
const code1 = `ubuntu@ubuntu1604:~/Documents/ctf/redpwn/2021/pick2$ python3.8 ./chall.py 
    ...
    return (
      ...
      <div className='main'>
        <div className='content'>
          <div><b>Description. </b>We're given a script that simply loads a pickle, prompts for a flag, and then outputs either 'Nope!' or 'Correct!'.
          </div>
          <Code>{code1}</Code>
          <div className='space'>The most straightforward way to inspect pickles is by using the the <span class='code-block'>dis</span> fuction from the <span class='code-block'>pickletools</span> module. We can then read the source, <span class='code-block'>eval</span> the relevant portion as bytes, and pass it to <span class='code-block'>dis</span>.
          </div>
          <Code>{code2}</Code>
          <div className='space'>For this challenge, we'll need to know a bit about pickle's bytecode spec. I'll try to explain as much as possible along the way, but if you need a more detailed explanation about the pickle VM, have a look <a href='https://checkoway.net/musings/pickle/'>here</a>, <a href='https://intoli.com/blog/dangerous-pickles/'>here</a>, and <a href='https://adrianstoll.com/computer-insecurity/python-in-a-pickle.html'>here</a>. Anyways, here's a part of <span class='code-block'>dis</span>'s output:
          </div>
          <Code>{code3}</Code>
          ...
          <div className='space'>
            Now, the problem is that <span class='code-block'>dis</span> writes to stdout, so there's no way for us to work with its output at runtime. As a quick workaround, we can write <span class='code-block'>dis</span>'s output to a file and then read from the file.
          </div>
          <Code>{code4}</Code>
          <div className='space'>Here's the output:</div>
          <Code>{code5}</Code>
          <div className='space'>The bytecode looks like its working with a bunch of pickle-themed variables. However, we cannot inspect these variables because it hasn't been loaded into memory yet (so far we've merely disassembled pickle). To actually run it, we need to load the pickle.
          </div>
          <Code>{code6}</Code>
          ...
          <Code>{code7}</Code>
          <div className='space'>Here's the tail:</div>
          <Code>{code8}</Code>
